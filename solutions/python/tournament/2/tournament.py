class Team:
    ...
    def __init__(self, name):
        ...
        self.name = name
        self.played, self.won = 0, 0
        self.drawn, self.lost, self.points = 0, 0, 0
...
def tally(results):
    output = create_line(Team.titles)
    if not results: return output

    for line in results.split('\n'): calc(line)

    sorted_teams = [v[1] for v in sorted(
        sorted([(name, team_ob, team_ob.points)
        for name, team_ob in Team.teams.items()]),
        key=lambda v: v[2], reverse=True)]
